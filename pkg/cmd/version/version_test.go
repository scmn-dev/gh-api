...
func TestChangelogURL(t *testing.T) {
	tag := "0.3.2"
	url := "https://github.com/secman-team/gh-api/releases/tag/v0.3.2"
	...
	tag = "0.3.2-pre.1"
	url = "https://github.com/secman-team/gh-api/releases/tag/v0.3.2-pre.1"
	result = changelogURL(tag)
	if result != url {
		t.Errorf("expected %s to create url %s but got %s", tag, url, result)
	}
	...
	url = "https://github.com/secman-team/gh-api/releases/latest"
	result = changelogURL(tag)
	if result != url {
		t.Errorf("expected %s to create url %s but got %s", tag, url, result)
	}
